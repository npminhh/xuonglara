...
try {
    ...
    public function edit(Product $product)
    {
        ...
        $sizes = ProductSize::query()->pluck('name', 'id')->all();
        $colors = ProductColor::query()->pluck('name', 'id')->all();
        return view(self::PATH_VIEW.__FUNCTION__, compact('categories', 'sizes', 'colors','product'));
...
//        dd($data);
        ...
        try {
            DB::beginTransaction();
            // tạo dữ liệu bảng product
            $product = Product::query()->update($data);
            // tạo dữ liệu cho bảng product variants
            foreach ($request->product_variants as $item) {
    ...
    {
        $sql=db::table('products')->where('id','=',$id)->delete();

        return redirect()->route('product.index')->with('msg', ' đã xóa thành công!');
